...
class P6(ProductionBase):
    def __init__(self):
        ...
        enode4 = self.graph.add_edge(node4, node8)
        enode5 = self.graph.add_edge(node4, node7)
        enode6 = self.graph.add_edge(node7, node1)
        enode7 = self.graph.add_edge(node6, node2)
        enode8 = self.graph.add_edge(node8, node3)
        
        self.enodes = [enode1, enode2, enode3, enode4, enode5, enode6, enode7, enode8]
    ...
    def apply_production(self, graph, mapping):
        ...
        graph.add_edge(mapping[self.nodes[7]], node)
        
        # Add hyperedges
        graph.add_q_node(mapping[self.nodes[3]], mapping[self.nodes[6]], node, mapping[self.nodes[7]])
        graph.add_q_node(mapping[self.nodes[7]], node, mapping[self.nodes[4]], mapping[self.nodes[2]])
        graph.add_q_node(mapping[self.nodes[6]], mapping[self.nodes[0]], mapping[self.nodes[5]], node)
        graph.add_q_node(node, mapping[self.nodes[5]], mapping[self.nodes[1]], mapping[self.nodes[4]])